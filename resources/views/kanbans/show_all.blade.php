...
<div class="flex flex-row justify-between h-16 border-b border-gray-200 pl-4 items-center flex-shrink-0">
    ...
    <div class="flex flex-wrap space-x-4 items-center">
        @foreach ($kanban->tags as $kanban_tag)
            <a href="{{ route('kanban.edit',$kanban->id) }}"
                class="flex flex-row my-2 focus:outline-none rounded text-sm p-1 {{ $kanban_tag->tag_color }} items-center">
            ...
            <div class="space-y-4 overflow-y-auto no-scrollbar">
                @foreach ($kanban_column->tasks as $kanban_task)
                        ...
                        <div class="group flex flex-col mx-1.5 bg-white rounded-md shadow hover:shadow-lg">
                            ...
                            <button class="flex flex-row my-2 focus:outline-none rounded text-sm p-1 {{ $kanban_task->KanbanTag->tag_color }} items-center">
                                <i class="{{ $kanban_task->KanbanTag->tag_icon }} transform -translate-y-0.5"></i>
                                <h2 class="px-1">{{ $kanban_task->KanbanTag->tag_name }}</h2>
                            </button>
